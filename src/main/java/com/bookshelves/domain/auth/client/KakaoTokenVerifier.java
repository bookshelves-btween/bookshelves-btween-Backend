...
public class KakaoTokenVerifier implements ProviderTokenVerifier {
  ...
  public KakaoTokenVerifier(
      ...
      @Value("${external.kakao.access-token-info-uri}") String accessTokenInfoUri,
      @Value("${external.kakao.app-id}") String appId) {
    this(buildRestClient(restClientBuilder, accessTokenInfoUri), appId);
  }

  KakaoTokenVerifier(RestClient restClient, String appId) {
    this.restClient = restClient;
    this.appId = appId;
  }

  private static RestClient buildRestClient(
      RestClient.Builder restClientBuilder, String accessTokenInfoUri) {
  ...
  public ProviderUserInfo verify(String providerToken) {
    KakaoAccessTokenInfoResponse response;
    ...
    if (response == null || response.id() == null || response.appId() == null) {
      throw new AuthException(AuthErrorCode.AUTH_INVALID_PROVIDER_TOKEN);
    }

    // 이 토큰이 우리 카카오 앱에 발급된 것인지 확인한다. 카카오는 같은 카카오계정이라도
    // 앱마다 다른 회원번호를 부여하므로, 이 확인 없이는 다른 앱에서 발급된 토큰의
    // 회원번호가 우리 DB의 기존 providerId와 우연히 겹칠 때 그 계정으로 로그인될 수 있다.
    if (!String.valueOf(response.appId()).equals(appId)) {
      throw new AuthException(AuthErrorCode.AUTH_INVALID_PROVIDER_TOKEN);
    }
  ...
  private record KakaoAccessTokenInfoResponse(Long id, @JsonProperty("app_id") Long appId) {}
}
